# Web UI/UX 专业知识工具设计（目标设计）
<!-- 文件名：web-ui-ux-knowledge-tool-design - Web界面知识工具设计 -->
<!-- 所属目录：design - 产品设计方案 -->

> 状态：**目标设计。** 目标不是让模型运行时随意搜索 GitHub，而是在开发阶段人工筛选、审查并版本化高质量知识，运行时从本地知识包中受控调用。
...
来源包括 MUI、Material 3、Carbon、Spectrum、Primer、Radix、Fluent、USWDS、GOV.UK、W3C/WCAG、Apple HIG 和 NN/g，入口见 [references - 设计参考与许可说明.md](references - 设计参考与许可说明.md)。
...
1. 对话页：输入需求、补充信息、人工确认；
2. 工作流页：节点、Tool、状态、失败和恢复；
3. 报告页：动态目录、候选差异、评审依据、导出；
4. Agent 管理页：角色、模型、权限、凭证；
5. 知识工具页：来源、版本、启用状态和适用场景。
