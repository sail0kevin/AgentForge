...
AgentForge 当前适合交付为“面向研发团队的需求规划与评审辅助 Web MVP”。它帮助产品经理、技术负责人和评审人把一段需求整理为结构化计划，检索指定知识库，生成 delivery/quality 两类候选方案，暴露风险与证据引用，并在关键冲突处暂停等待人工决策。
...
3. 系统生成计划和双候选评审，用户检查证据、风险、冲突和干预信号。
4. 用户逐项批准或修改允许修改的计划任务。
5. Reporter 生成带版本和来源的 Markdown 报告，用户人工确认后用于评审会议或研发排期。
