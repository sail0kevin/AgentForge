...
REPORT-3也已完成：`/workflows` 将 Planner、双候选、Reviewer、Evaluator、人工确认和 Reporter 合并到同一个产品级 LangGraph；缺失信息和高影响取舍可通过持久 Checkpoint 暂停、恢复，异常失败还可以在租约和乐观锁保护下继续。baseline和model共用同一状态图，完整Checkpoint不会发送到浏览器。详细设计、恢复语义、失败实验和测试证据见[产品工作流与Checkpoint技术专题](./2026-07-15 - phase-6-workflow-checkpoint-completion - 工作流与Checkpoint恢复.md)。
...
- `prisma/schema.prisma`、`prisma/postgres/schema.prisma`：ReportArtifact关系；
