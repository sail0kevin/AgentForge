...
export function SequenceDashboard({ t, agents, messages, totalSpent, budgetStatus }: { t: Copy; agents: LocalAgent[]; messages: WorkspaceMessage[]; totalSpent: number; budgetStatus: string }) {
  ...
  const dashboardText = {
    ...
    messageCount: isZh ? "数据库运行事件" : "Database run events",
    userMessages: isZh ? "需求提交次数" : "Requirement submissions",
    assistantMessages: isZh ? "Agent 运行结果" : "Agent run results",
    inputTokens: isZh ? "输入 Token" : "Input tokens",
    outputTokens: isZh ? "输出 Token" : "Output tokens",
    databaseCost: isZh ? "数据库累计费用" : "Database cost",
    currentRunCost: isZh ? "当前运行费用" : "Current run cost",
    ...
    localOverviewDesc: isZh ? "这里显示当前页面已加载的 Agent 与运行事件，用于和数据库统计互相校验。" : "Loaded agents and run events in the current browser session for comparison.",
    loadedAgents: isZh ? "已加载 Agent" : "Loaded agents",
    visibleMessages: isZh ? "当前可见运行事件" : "Visible run events",
