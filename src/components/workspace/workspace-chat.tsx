...
export function ChatWorkspace(props: {
  ...
  return (
    <section className="flex h-[calc(100vh-132px)] min-h-[620px] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      <aside className="w-[300px] shrink-0 overflow-y-auto border-r border-slate-200 bg-[#FAFBFC] p-3 max-lg:w-[72px]">
        ...
        <div className="space-y-2">
          {props.workspaces.map((workspace) => <button key={workspace.id} type="button" onClick={() => props.onSelectWorkspace(workspace.id)} className={cn("w-full rounded-lg border p-3 text-left transition", workspace.id === props.activeWorkspace?.id ? "border-[#5B5BD6] bg-indigo-50" : "border-slate-200 bg-white hover:bg-slate-50")}><div className="truncate text-sm font-medium text-slate-900">{workspace.name}</div><div className="mt-1 truncate text-xs text-slate-500">{workspace.description || props.t.workspaceNoDescription}</div><div className="mt-2 text-[11px] text-slate-500">{workspace.agents.length} {props.t.workspaceAgentCount}</div></button>)}
...
function MessageBubble({ t, message, agent }: { t: Copy; message: WorkspaceMessage; agent?: AgentConfig }) {
  ...
  return <div className={cn("flex", isUser ? "justify-end" : "justify-start")}><div className={cn("max-w-[76%] rounded-lg border p-4 text-sm leading-6", isUser ? "border-slate-200 bg-slate-100" : isFailed ? "border-red-200 bg-red-50 shadow-sm" : "border-slate-200 bg-white shadow-sm")}><div className="mb-2 flex items-center gap-2 text-xs text-slate-500">{!isUser && agent && <Avatar agent={agent} small />}<span>{isUser ? t.you : agent?.name ?? "AI"}</span>{isFailed && <span className="rounded bg-red-100 px-1.5 py-0.5 text-[10px] font-medium text-red-600">{t.callFailed}</span>}</div><p className="whitespace-pre-wrap text-slate-700">{message.content}</p></div></div>;
}

function ThinkingBubble({ t, agent }: { t: Copy; agent?: AgentConfig }) {
  return <div className="flex justify-start" role="status" aria-live="polite"><div className="rounded-lg border border-indigo-100 bg-white p-4 text-sm text-[#5B5BD6] shadow-sm"><div className="flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />{agent?.name ?? "AI"} {t.thinking}</div></div></div>;
}
