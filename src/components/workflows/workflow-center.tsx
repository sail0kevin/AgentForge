    ...
    <main className="secondary-page min-h-screen text-slate-900">
      ...
      <div className="mx-auto grid max-w-[1500px] gap-5 p-5 lg:grid-cols-[320px_minmax(0,1fr)_340px]">
        ...
        <aside className="space-y-4">
          ...
          {selected && <section className="secondary-card rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-center gap-2"><ShieldCheck className="text-emerald-600" /><h2 className="font-bold">Artifact链</h2></div><div className="mt-3 space-y-2 text-sm"><div className="soft-card rounded-lg bg-slate-50 p-3"><b>PlanningArtifact</b><p className="mt-1 text-xs text-slate-500">{selected.artifacts.plan ? `${selected.artifacts.plan.status} · ${selected.artifacts.plan.id}` : "尚未创建"}</p></div><div className="soft-card rounded-lg bg-slate-50 p-3"><b>ReviewWorkflow</b><p className="mt-1 text-xs text-slate-500">{selected.artifacts.review ? `${selected.artifacts.review.status} · ${selected.artifacts.review.id}` : "尚未创建"}</p></div><div className="soft-card rounded-lg bg-slate-50 p-3"><b>ReportArtifact</b>{selected.artifacts.report ? <><p className="mt-1 text-xs text-slate-500">v{selected.artifacts.report.version} · {selected.artifacts.report.status}</p><Link href="/reports" className="mt-2 inline-flex text-xs font-bold text-violet-700">查看报告 →</Link></> : <p className="mt-1 text-xs text-slate-500">尚未创建</p>}</div></div></section>}
