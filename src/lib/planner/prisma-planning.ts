...
export async function savePlanningArtifact(input: ArtifactIdentity & { result: PlannerResult }) {
  ...
  return prisma.planningArtifact.upsert({
    ...
    update: {
      ...
      complexityScore: input.result.status === "ready" ? input.result.complexityScore ?? null : null,
      recommendedCandidates: input.result.status === "ready" ? input.result.recommendedCandidates ?? null : null,
      failureCode: null,
